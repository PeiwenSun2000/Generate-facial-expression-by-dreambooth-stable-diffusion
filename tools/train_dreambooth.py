...
def get_label_from_txt(path,label_path=None):
    if label_path:
        path = Path(os.path.join(label_path,os.path.basename(path)))
...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        prompt = get_label_from_txt(path,"/mnt/tempDisk/BP4D-256-crop/enhanced-au-nl-label") if self.use_txt_as_label else prompt
        ...
        # print(example["class_prompt_ids"],prompt)
        return example
...
def main(args):
    ...
    accelerator = Accelerator(
        ...
        kwargs_handlers=[InitProcessGroupKwargs(timeout=timedelta(seconds=54000))],
    )
